    ...
    switch ($step) {
        default:
            ...
            if (!is_writeable($dbFile)) {
                $error = true;
                echo "<div class='alert alert-error'><i class='icon-remove'></i> Database File (app/config/database.php) is not writeable!</div>";
            }
            if (phpversion() < '7.2') {
                $error = true;
                echo "<div class='alert alert-error'><i class='icon-remove'></i> Your PHP version is " . phpversion() . '! PHP 7.2 or higher required!</div>';
            } else {
        ...
        case '0':
            ...
            if ($_POST) {
                ...
                $referer = $protocol . $_SERVER['SERVER_NAME'] . substr($_SERVER['REQUEST_URI'], 0, -24);
            ...
            <?php if ($_POST) { ?>
                <form action="index.php?step=4" method="POST" class="form-horizontal">
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <input type="text" id="domain" name="domain" class="xlarge" required data-error="Base URL is required" value="<?php echo $protocol . $_SERVER['SERVER_NAME'] . substr($_SERVER['REQUEST_URI'], 0, -24); ?>" />
                        </div>
                    </div>
                    <div class="control-group">
        ...
        case '5':
            ...
            if ($_POST) {
                ...
                if ($object->status == 'success') {
                    ...
                    if ($database->create_tables($dbdata, $_POST['username'], $_POST['code']) == false) {
                        ...
                        $finished = true;
                        if (!@unlink('../SIM3')) {
                            echo "<div class='alert alert-warning'><i class='icon-warning'></i> Please remove the SIM3 file from the main folder in order to lock the installer.</div>";
                        }
                    }
                } else {
            ...
            if ($finished) {
                ...
                <div class="bottom">
                    <a href="<?php echo $protocol . $_SERVER['SERVER_NAME'] . substr($_SERVER['REQUEST_URI'], 0, -24); ?>" class="btn btn-primary">Go to Login</a>
                </div>
