...
class Tec_dompdf extends DOMPDF
{
    ...
    public function generate($content, $name = 'download.pdf', $output_type = null, $footer = null, $margin_bottom = null, $header = null, $margin_top = null, $orientation = 'P')
    {
        ...
        $this->getOptions()->setIsFontSubsettingEnabled(true)->setIsPhpEnabled(true)->setIsHtml5ParserEnabled(true)->setIsRemoteEnabled(true);
        $this->render();

        if ($output_type == 'S') {
            $output = $this->output();
            write_file('uploads/' . $name, $output);
            return 'uploads/' . $name;
        }
        $this->stream($name);
        return true;
