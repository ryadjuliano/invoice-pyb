    ...
    switch ($step) {
        ...
        case '0':
            ...
            if ($_POST) {
                ...
                $referer = $protocol . $_SERVER['SERVER_NAME'] . substr($_SERVER['REQUEST_URI'], 0, -23);
            ...
            <?php if ($_POST) { ?>
                <form action="index.php?step=2" method="POST" class="form-horizontal">
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <input type="text" id="domain" name="domain" class="xlarge" required data-error="Base URL is required" value="<?php echo $protocol . $_SERVER['SERVER_NAME'] . substr($_SERVER['REQUEST_URI'], 0, -23); ?>" />
                        </div>
                    </div>
                    <div class="control-group">
        ...
        case '3':
                ...
                if ($object->status == 'success') {
                    if (isset($object->database)) {
                        ...
                        if ($database->update_db($dbdata, $_POST['username'], $_POST['code']) == false) {
                            ...
                            $finished = true;
                            if (!@unlink('../SIM3')) {
                                echo "<div class='alert alert-warning'><i class='icon-warning'></i> Please remove the SIM3 file from the main folder in order to lock the update tool.</div>";
                            }
                        }
                    } else {
            ...
            if ($finished) {
                ...
                <div class="bottom">
                    <a href="<?php echo $protocol . $_SERVER['SERVER_NAME'] . substr($_SERVER['REQUEST_URI'], 0, -23); ?>" class="btn btn-primary">Go to Login</a>
                </div>
